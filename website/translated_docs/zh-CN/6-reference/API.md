...
| buy_price | double | 此刻购买 RAM 的价格，以 AIOU/byte 为单位，已经包含手续费 |
| sell_price | double | 此刻购买 RAM 的价格，以 AIOU/byte 为单位| 
...
| amount_limit |repeated [AmountLimit](#amountlimit)  | 用户可以设置的花费token的限制, 如 {"aiou": 100} 即本次交易对于每个签名者花费aiou不超过100 |
...
开发者可以使用 [Javascript SDK](https://github.com/aiou-official/aiou.js) 中的函数接口发送交易。
...
* **交易结构体转字节数组**
	...
	| string | 将字符串中每个字符对应的字节进行拼接并在前面加上长度 | 如 "aiou" 对应的字节数组为 [0 0 0 4 105 111 115 116] |
	| 数组 | 将数组的每个元素转为字节数组并拼接，再在其前面加上数组的长度  | 如 ["aiou" "aiou"] 对应的字节数组为 [0 0 0 2 0 0 0 4 105 111 115 116 0 0 0 4 105 111 115 116] |
	| map | 将字典的每对 key:value 分别转成字节数组并拼接, 然后按照 key 升序排列后拼接每一对，再在其前面加上 map 的长度 | 如 ["b":"aiou", "a":"aiou"] 对应的字节数组为 [0 0 0 2 0 0 0 1 97 0 0 0 4 105 111 115 116 0 0 0 1 98 0 0 0 4 105 111 115 116] |
...
"delay"、"chain\_id"、"reserved"、"signers"、"actions"、"amount\_limit"、"signatures"，所以交易结构体转字节数组伪代码为：
	...
	golang 的实现可参考 [go-aiou](https://github.com/aiou-official/go-aiou/blob/master/sdk/utils.go#L194)。 javascript 的实现可参考 [aiou.js](https://github.com/aiou-official/aiou.js/blob/master/lib/structs.js#L108)。
